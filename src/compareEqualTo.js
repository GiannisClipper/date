import compare from './compare.js';

/**
* Compares each date of a series of dates to be equal to the others
* 
* @param { ...( string | Object[] ) } dates - A series of dates
* 
* @returns { boolean } - True or false or null on invalid params
* 
* @example
* compareEqualTo( '20210101', '20210101' );  // returns true
* compareEqualTo( '20210101', '20210101', '20210102' );  // returns false
*/

const compareEqualTo = ( ...dates ) => {

    return compare( dates, ( date1, date2 ) => date1 === date2 );
};

export default compareEqualTo;
export { compareEqualTo };